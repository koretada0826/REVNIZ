    ...
    <div className="space-y-6 animate-in">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="body-sm mt-1.5">困っていること、探していることを投稿し、接点のきっかけを作る</p>
        </div>
      ...
      <div className="flex gap-0 border-b border-line">
        {[{ l: "新着", i: MessageSquare }, { l: "人気", i: TrendingUp }, { l: "締切間近", i: Clock }].map((t, idx) => (
          <button key={t.l} className={`flex items-center gap-1.5 px-4 py-3 text-[13px] font-medium border-b-2 transition-colors ${idx === 0 ? "border-red text-black-900" : "border-transparent text-muted hover:text-black-900"}`}>
        ...
        {consultations.map((p) => (
          <div key={p.id} className="card card-hover group cursor-pointer">
            <div className="flex items-start gap-4">
              <div className="w-9 h-9 rounded bg-black-50 flex items-center justify-center shrink-0 mt-0.5"><MessageSquare className="w-4 h-4 text-black-300" /></div>
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-bold text-black-900 text-[14px] mb-1 group-hover:text-red transition-colors">{p.title}</h3>
                <p className="body-sm line-clamp-2 mb-2">{p.content}</p>
                <div className="flex items-center gap-3 caption">
                  <span className="font-semibold text-black-900">{p.companyName}</span>
                  <span className="flex items-center gap-1"><MapPin className="w-3 h-3" /> {p.targetArea}</span>
                  <span>{p.createdAt}</span>
                </div>
              </div>
              <div className="flex flex-col items-center gap-1 shrink-0 pt-1">
                <p className="text-[18px] font-bold text-black-900">{p.responses}</p>
                <p className="text-[9px] text-black-300 uppercase tracking-wide">反応</p>
                <ChevronRight className="w-4 h-4 text-black-200 group-hover:text-red transition-colors mt-1" />
              </div>
            </div>
          </div>
        ))}
